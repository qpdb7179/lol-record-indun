...
function getSummonerByPuuid(puuid) {
  const url = `https://${PLATFORM_ROUTE}.api.riotgames.com/lol/summoner/v4/summoners/by-puuid/${puuid}`;
  return riotFetch(url); // { id, puuid, profileIconId, summonerLevel, ... }
}

function getLeagueEntriesBySummoner(summonerId) {
  const url = `https://${PLATFORM_ROUTE}.api.riotgames.com/lol/league/v4/entries/by-summoner/${summonerId}`;
...
async function fetchPlayerProfile(gameName, tagLine) {
  ...
  const [entries, topMasteries] = await Promise.all([
    getLeagueEntriesBySummoner(summoner.id),
