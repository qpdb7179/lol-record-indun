...
async function riotFetch(url) {
  ...
  if (!res.ok) {
    // 429(레이트리밋)는 사용자에게 원문 에러 대신 이해 가능한 문구로 바꿔서 던짐 — 이 키가 기본 티어(100req/120s)라 자주 발생할 수 있음
    if (res.status === 429) {
      const retryAfter = res.headers.get('Retry-After');
      const wait = retryAfter ? `${retryAfter}초` : '잠시';
      throw new Error(`요청이 몰려서 라이엇 서버가 잠시 응답을 제한하고 있어요. ${wait} 후 다시 시도해주세요.`);
    }
...
const RECENT_GAMES_COUNT = 10; // "최근 기록"(전체 큐, 판별 스코어보드 포함) 기본값
const RANKED_QUEUE_GAMES_COUNT = 20; // 솔로/자유랭크 챔피언별 성적 집계 기준(판별 상세 없이 집계만이라 좀 더 넉넉하게)

function getMatchIdsByPuuid(puuid, count, queueId) {
  // match-v5는 account-v1과 같은 지역(REGION_ROUTE) 라우팅을 씀 — kr이 아니라 asia
  // queueId를 주면 그 큐(예: 420=솔로랭크, 440=자유랭크)의 경기만 필터링해서 받아옴
  const queueParam = queueId ? `&queue=${queueId}` : '';
  const url = `https://${REGION_ROUTE}.api.riotgames.com/lol/match/v5/matches/by-puuid/${puuid}/ids?start=0&count=${count}${queueParam}`;
  return riotFetch(url); // [matchId, ...]
}

const RANKED_QUEUE_ID = { solo: 420, flex: 440 };
...
// 최근 N경기 — 챔피언별 집계(perChampion) + op.gg 스타일 개별 경기 목록(perGame) 둘 다 반환.
// queueId를 주면 그 큐(솔로/자유랭크)로만 필터링(참가자 상세 모달의 랭크 카드 클릭 시 사용).
// 호출량: 1(목록) + N(상세). 같은 match-v5 응답에서 두 형태를 동시에 뽑아내서 추가 호출은 없음.
async function fetchRecentChampionStats(puuid, count = RECENT_GAMES_COUNT, queueId = null) {
  const matchIds = await getMatchIdsByPuuid(puuid, count, queueId);
...
module.exports = { fetchPlayerProfile, fetchRecentChampionStats, RECENT_GAMES_COUNT, RANKED_QUEUE_GAMES_COUNT, RANKED_QUEUE_ID };
