...
async function fetchRecentChampionStats(puuid, count = RECENT_GAMES_COUNT) {
  ...
  for (const match of matches) {
    ...
    perGame.push({
      ...
      myTeam: match.info.participants.filter((x) => x.teamId === myTeamId).map((x) => x.championId),
      enemyTeam: match.info.participants.filter((x) => x.teamId !== myTeamId).map((x) => x.championId),
    });
  }
